...
class WeeklyAggregator(object):
    def __init__(self, calendar: dataframe, sales: dataframe, product: dataframe, store: dataframe):
    ...
    def _build_full_df(self) -> dataframe:
        """
        This method builds a `pyspark.sql.dataframe` that contains all the possible sales on the website by creating a
        `crossJoin` between `product`, `calendar` and `store` dataframes
        :return: A dataframe.
        """
    ...
    def aggregate_per_week(self) -> dataframe:
        ...
        # `fillna(0)` is chosen because a `np.nan` simply means that there was no sales for the tuple
